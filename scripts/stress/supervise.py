"""Autonomous babysitting mode for the full 12-run supplementary-stress
sequence.
...
silently without a trace.
    ...
    python -m scripts.stress.supervise
"""
...
def main() -> None:
    ...
    print(f"=== Supervised stress run starting at {started}: 12 slots, up to {MAX_RETRIES} attempts each ===")

    for slot in run_order():
        label = f"{slot.system} {slot.workload} r{slot.repetition} (position {slot.position}/12)"
    ...
    print(f"=== Supervised run finished at {finished}: all 12 slots valid ===")
